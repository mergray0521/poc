...
with st.container():
    for col, token in zip([c1, c2, c3, c4, c5, c6], tokens):
        with col:
            ...
            col.markdown(f'<div class="custom-container"><p>Utility: {token["utility"]}</p><img src="{token["image_url"]}" alt="{token["name"]}" class="custom-image"><p>{token["name"]} - {token["token_cost"]} points</p></div>', unsafe_allow_html=True)
