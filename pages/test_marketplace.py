...
# Move the st.button block outside the container loop
if st.button("Purchase Selected Token"):
    # Handle purchase logic here, e.g., you can check which token is selected and perform the purchase
    selected_token = tokens[0]  # You need to replace this with the logic to determine the selected token
    handle_purchase(selected_token["token_id"], selected_token["token_cost"])
